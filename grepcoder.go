...
// Help
func PrintHelp() {
	fmt.Println("Usage: \n\tgo run crawler.go <startId> <endId> <comma-separated keywords>")
}
...
var (
	...
	problemId = []string { "d", "e", "f" }		// Add Problem a, b, c, g
	prefix  = "https://atcoder.jp/contests/abc"
	midfix  = "/tasks/abc"
	suffix  = "/editorial"
	EdUrl   []string
	keyword []string
...
func generateProblemURLs(startId, endId int) []string {
	...
	for i := startId; i <= endId; i++ {
		for _, Id := range problemId{
			ids = append(ids, fmt.Sprintf("%s%d%s%d_%s%s", prefix, i, midfix, i, Id, suffix))
		}
	}
	return ids
}
